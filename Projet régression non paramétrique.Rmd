---
title: "Projet régression non paramétrique"
...
## chargement de jeu de données Data

```{r}
data=read.table("C:/Users/HP/Desktop/regression non param moodle/Data.txt")
summary(data)
plot(data$X, data$Y, xlab = "X", ylab = "Y", main = "Y = f(X)")
```

On observe sur le graphe de la variation de Xi en fontion Yi: deux bosses et une vallée.
...
```{r}
g_enp1 = bkde(data$X, kernel = "normal",canonical = F, bandwidth = 0.01, truncate = F)
g_enp2 = bkde(data$X, kernel = "normal", canonical = F, bandwidth = 0.1, truncate = F)
g_enp3 = bkde(data$X, kernel = "normal",canonical = F, bandwidth = 0.5, truncate = F)
g_enp4 = bkde(data$X, kernel = "normal",canonical = F, bandwidth = 1, truncate = F)
plot(g_enp1$x, g_enp1$y, type = "l", col = "red", xlim = c(1,6),ylim=c(-2,8))
...
```{r}
g_enp = bkde(data$X, kernel = "normal", canonical = F, bandwidth = hn, truncate = F)
xunif = runif(10000)
plot(g_enp$x, g_enp$y, type = "l", col = "blue", xlim = c(1,6), main = "Estimateur de g VS Loi uniforme sur [0,8]")
lines(g_enp$x, dunif(g_enp$x,0,8), type = "l", col = "red")
legend(3,0.32, legend =c("Estimateur de g","densité de loi uniforme"), col = c("blue", "red"), lty = 1)
```
```{r}
...
```{r}
qqplot(data$X, runif(10000), main = "QQplot : Xi / Loi Uniforme sur [0,8]")
```
```{r}
...
plot(data$X, data$Y, xlab = "X", ylab = "Y", main = "Données VS Droite de régression Y~X")
abline(reg, col = "red")
legend(0.8,12, legend =c("Données","Droite de régression Y~X"), col = c("black", "red"), lty = 1)
```
...
- l'hypothèse d'homoscédasticité n'est pas totalement vérifiée (la répartition des résidus est différente sur [3.5,6.3] dans le graphique "Scale-Location").
```{r}
par(mfrow=c(2,2))
plot(reg)
```
...
n=length(X)
std=sqrt(var(X))
hselect = h.select(data$X, method = "cv")
h_dpill=dpill(data$X,data$Y)
...
```
Nous remarquons que :
- les différents estimateurs ont un comportement similaire sur l'intervalle [1,6] et sont performants,
- l'estimateur de NW (avec h = hselect) est nettement moins performant sur l'intervalle [1,6] que les 4 autres,
- les estimateurs par polynômes locaux de degré 2 *LP Pill* et *LP silver* semblent les plus performants sur l'intervalle [1, 6].
La différence de performance sur ces deux intervalles est liée à la quantité de données qui diffèrent sur ces deux intervalles. En effet, l'intervalle [1,6] est moins fourni en données. A h constant pour tout x, les estimateurs sont donc moins précis sur cet intervalle (la variance étant de l'ordre de 1/nh).
```{r}
demoKSR(data$X, data$Y)
```
...
```{r Calcul de Un}
Un = sum((data$Y[2:length(data$Y)] - data$Y[1:(length(data$Y)-1)])^2)/ (2*length(data$Y) - 2)
```
```{r Calcul de Vn}
```
...
```{r}
varianceY = 1/(4999) * sum((data$Y[1:5000] - mean(data$Y))^2)
```
Nous remarquons que celle-ci est égale Un.

Nous remarquons également que la valeur de Vn est très proche de celle de la variance du bruit epsilon.
```{r}
...
X.1=data[1:5000,]
library(dplyr)
data1= X.1=data[1:2500,]
data2 = X.1=data[2501:5000,]
```
Nous construisons un estimateur de R à l'aide de data1 et nous calculons sa valeur aux points xi de data2.
```{r}
R_moins=ksmooth(data1$X, data1$Y, bandwidth=dpill(data1$X,data1$Y), x.points=data2$X)
plot(R_moins, main = "Estimateur de R aux points Xi de data2")
```
...
```{r}
set.seed(2)
Y_Tilde = data2$Y[order(data2$X)] - R_moins$y
plot(density(Y_Tilde), col = "blue", main = "Distribution de Y_Tilde VS Loi Normale")
lines(density(rnorm(2500,0,1)), col = 'red')
```
```
